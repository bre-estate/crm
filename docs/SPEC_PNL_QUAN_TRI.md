...
- Tách loại thuế trên sao kê: `scripts/import-tax-receipts.ts` đọc giấy nộp thuế PDF trong `6.1-Hồ sơ thuế` (mã tiểu mục 1701 GTGT, 1001 TNCN, 1052 TNDN, 2863 môn bài) vào bảng `tax_payments` (drizzle/0043); dòng KBNN trên sao kê khớp ngày (±3) và số tiền thì ra đúng loại, không khớp để `thue_kbnn`. `0.2.5-Sổ theo dõi nộp thuế.xlsx` trên Drive trống, không dùng được. 2026 còn 251,7tr chưa có giấy nộp (65tr và 177,65tr đầu năm, vài khoản lẻ).
- Nguồn sao kê (từ 14/09/2026): `data-excel/sao ke/<năm>/<quý>.xlsx`, bản Admin xuất theo quý từ Internet Banking. `scripts/import-sao-ke.ts` đọc cả .xlsx lẫn .csv, xếp file theo ngày giao dịch đầu tiên, gán `statement_seq` rồi chuẩn hóa lại theo ngày. Dòng phí ngân hàng để tiền ở cột Phí và Thuế, cột Nợ bằng 0, nên số tiền thật của một dòng = nợ + có + phí + thuế (đã sửa ở `loadCashLegsFromBank`, `loadBankBalance` và luật `dich_vu_ngoai` trong bank-cash-core). Sau khi nạp: 838 dòng, 17/10/2024 đến 03/08/2026. Chuỗi số dư khớp tuyệt đối cho 2024 và 2026. Còn thiếu 26–31/12/2025 (159.169.123, gồm 159.077.240 T&A ngày 31/12 mà sổ NKC có) và cả quý 3/2026 (tháng 7 và 8 hiện lấy từ bản CSV cũ, tháng 9 chưa có). Bảy chỗ "đứt chuỗi" trong cùng ngày ở 09/02/2026 và 04/06/2026 là do ngân hàng xếp dòng trong ngày không theo thứ tự, cộng lại bằng 0, không phải thiếu tiền.
- Sao kê có đủ dòng không (kiểm 13/09/2026 bằng chuỗi số dư trên CSV Techcombank, không OCR): sao kê là file CSV xuất từ Internet Banking, `scripts/import-sao-ke.ts` ghi thêm `statement_seq` (thứ tự dòng, lớn = mới) và khóa duy nhất theo (số bút toán, nợ, có) vì Techcombank dùng lại số bút toán cho lệnh hoàn (BHXH 05/03 và 17/03/2026, hai lệnh nộp 21.721.243 và 18.400.000 từng bị rơi). Trang báo cáo có khối "Đối chiếu với số dư trên sao kê": số dư đầu kỳ (dòng có seq nhỏ nhất trừ số tiền của chính nó), cuối kỳ (seq lớn nhất), thay đổi theo sao kê so với tổng dòng đã nạp, lệch nghĩa là thiếu dòng. Kết quả: 2026 đầu kỳ 1.568.313.197 (05/01) cuối kỳ 1.044.056.421 (03/08), tổng dòng lệch 346.500 (một dòng phí sao kê 09/06 không có trong file xuất); 2025 lệch 44.000 (phí phát hành séc 05/02). Chỗ thiếu hẳn giữa hai file: 26/12 đến 31/12/2025 (159.169.123, gồm 159.077.240 T&A ngày 31/12 mà sổ NKC có) và tháng 11/2024 (13.736.813). Cần xuất lại sao kê hai khoảng đó. Tiền gửi tiết kiệm có kỳ hạn (900tr, 21/07/2026) là chuyển nội bộ, khối đối chiếu cộng lại để thấy tổng tiền công ty.
- Tiền vào từ người có tên trong `products.customer_name` (kể cả viết liền "ThomNguyen") là giữ chỗ.
- Lệnh gộp lương + hoa hồng (chỉ T12/2025 và T1/2026, từ T2/2026 kế toán chuyển lương riêng): phần lương = lương cơ bản + phụ cấp trong bảng lương tháng đó (`payroll_months`, drizzle/0044, `scripts/import-payroll.ts` đọc sheet 3-Bangluong-HĐLĐ và Bangluong-CTV), tháng lấy từ diễn giải "T12 2025". Không dùng Báo cáo doanh thu để tách vì HH trong BCDT là số gộp trước thuế TNCN, sao kê trả số thực nhận, trừ thẳng ra âm ở 20/45 lệnh (kiểm 12/09/2026).
